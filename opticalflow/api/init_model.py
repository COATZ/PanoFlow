...
from opticalflow.core.model import csflow, raft, raft_sphe, panoflow_csflow, panoflow_csflow_sphe, panoflow_raft
...
def init_RAFT_sphe(args):
    if args.train:
        if args.change_gpu:
            device = torch.device(('cuda:' + str(args.gpus[0])))
            model = raft_sphe.RAFT_sphe(args)
            model.to(device)
            print('Parameter Count: %d' % count_parameters(model))
            ...
            model = torch.nn.DataParallel(
                raft_sphe.RAFT_sphe(args), device_ids=args.gpus)
            print('Parameter Count: %d' % count_parameters(model))
        ...
        return raft_sphe.RAFT_sphe(args)
...
def init_PanoCSFlow_sphe(args):
    if args.train:
        if args.change_gpu:
            device = torch.device(('cuda:' + str(args.gpus[0])))
            model = panoflow_csflow_sphe.PanoCSFlow_sphe(args)
            model.to(device)
            print('Parameter Count: %d' % count_parameters(model))
            ...
            model = torch.nn.DataParallel(
                panoflow_csflow_sphe.PanoCSFlow_sphe(args), device_ids=args.gpus)
            print('Parameter Count: %d' % count_parameters(model))
        ...
        return panoflow_csflow_sphe.PanoCSFlow_sphe(args)
...
def init_model(args):
    ...
    elif args.model == 'RAFT_sphe':
        return init_RAFT_sphe(args)
    elif args.model == 'PanoFlow(CSFlow)':
        return init_PanoCSFlow(args)
    elif args.model == 'PanoFlow(CSFlow)_sphe':
        return init_PanoCSFlow_sphe(args)
