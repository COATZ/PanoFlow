...
class DeformConv2d_sphe2(nn.Module):
    ...
    def forward(self, input: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        if self.init == 0:
            ...
            print("python3 opticalflow/core/model/external/create_offset_tensor.py --w {} --h {} --k {} --s {} --p {} --d {}".format(input.shape[-1], input.shape[-2], self.kernel_size[0], self.stride[0], self.padding[0], self.dilation[0]))
