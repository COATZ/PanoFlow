...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='CSFlow',
        choices=['CSFlow', 'RAFT', 'RAFT_sphe', 'PanoFlow(CSFlow)', 'PanoFlow(CSFlow)_sphe', 'PanoFlow(RAFT)'])
    parser.add_argument(
        '--CFE',
    ...
    parser.add_argument(
        ...
        default="/media/cartizzu/DATA/DATASETS/Flow360_release/")
    parser.add_argument(
        '--val_Omni_root',
        help='Root of the current datasets',
        default="/media/cartizzu/DATA/DATASETS/OMNIFLOWNET_DATASET/")
    parser.add_argument(
        '--validation',
        type=str,
        nargs='+',
...
def main():
    ...
    add_args = "M"
    ckpt_name = args.restore_ckpt.split("-")[-1].split(".")[0]
    # print(str(args.validation[0]))
    run_name = os.path.join('./OUTPUT/', 'EVAL_'+str(args.validation[0])+'_'+args.model+'_'+ckpt_name+'_'+add_args+'_'+str(args.CFE)+'_'+str(args.cvt_gt)+'.txt')
    print(run_name)
    save_file = open(run_name, 'w')
    save_file.write(args.restore_ckpt)
    save_file.write('\n\n')
    ...
    # print(results)
    print(results, file=save_file)
    # save_file.write(pickle.dumps(results))
